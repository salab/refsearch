...
export const ingestRMinerFile = async (repoUrl: string): Promise<void> => {
  ...
  console.log(`[reader > rminer] Processed ${output.commits.length} commits and inserted ${res.insertedCount} refactoring instances in ${formatTime(start)}.`)
}
...
export const ingestRefDiffFile = async (repoUrl: string): Promise<void> => {
  ...
  console.log(`[reader > refdiff] Processed ${output.length} commits and inserted ${res.insertedCount} refactoring instances in ${formatTime(start)}.`)
}
